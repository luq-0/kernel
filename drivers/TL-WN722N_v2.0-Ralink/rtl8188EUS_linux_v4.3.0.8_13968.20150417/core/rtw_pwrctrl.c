...
#else
	struct dvobj_priv *pdvobj = container_of(t, struct dvobj_priv, pwrctl_priv.pwr_state_check_timer);
	_adapter *padapter = container_of(&pdvobj, _adapter, dvobj);
#endif
	rtw_ps_cmd(padapter);
}
...
#else
	padapter = from_timer(padapter, t, pwrctrlpriv.pwr_rpwm_timer);
#endif
	pwrpriv = adapter_to_pwrctl(padapter);
	DBG_871X("+%s: rpwm=0x%02X cpwm=0x%02X\n", __func__, pwrpriv->rpwm, pwrpriv->cpwm);
